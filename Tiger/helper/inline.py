...
from Tiger import ids as list_users
...
def cb_wrapper(func):
    async def wrapper(client, cb):
        ...
        if cb.from_user.id not in users:
            await cb.answer(
                "ᴋʏᴀ ʜᴀɪ? ᴋᴀᴀᴍ kar apna ʙᴀᴅᴀ ᴀᴀʏᴀ !",
                cache_time=0,
                show_alert=True,
            )
        else:
            try:
                ...
                print(format_exc())
                await cb.answer(
                    f"ᴏʜ ɴᴏ, sᴏᴍᴇᴛʜɪɴɢ ɪsɴ ʀɪɢʜᴛ. ᴘʟᴇᴀsᴇ ᴄʜᴇᴄᴋ ʟᴏɢs!",
                    cache_time=0,
                    show_alert=True,
                )
        ...
        if inline_query.from_user.id not in users:
            await client.answer_inline_query(
                ...
                results=[
                    (
                        InlineQueryResultArticle(
                            title="sᴏʀʀʏ, ᴀᴡᴡ ʙᴀʙᴇ ʏᴏᴜ ᴄᴀɴ ᴜsᴇ ᴍᴇ!",
                            input_message_content=InputTextMessageContent(
                                "ʏᴏᴜ ᴄᴀɴɴᴏᴛ ᴀᴄᴄᴇss ᴛʜɪs ʙᴏᴛ , ᴍᴀᴋᴇ ʏᴏᴜʀ ᴏᴡɴ ʙᴏᴛ ʙᴀʙᴇ"
                            ),
                        )
                    )
                ],
