...
from Tiger import CMD_HELP, app
from Tiger.helper.data import Data
from Tiger.helper.inline import cb_wrapper, paginate_help
from Tiger import ids as users
...
async def on_plug_in_cb(_, callback_query: CallbackQuery):
    ...
    this_command = f"──「 **нєℓρ fσя {str(modul_name).upper()}** 」──\n\n"
    for x in commands:
        this_command += f"  •  **¢σммαи∂:** `.{str(x)}`\n  •  **fυи¢тισиѕ:** `{str(commands[x])}`\n\n"
    ...
    reply_pop_up_alert = (
        ...
        else f"{module_name} иσ ∂σ¢υмєитαтισи нαѕ вєєи ωяιттєи fσя тнє мσ∂υℓє."
    )
    await app.edit_inline_text(
        callback_query.inline_message_id,
        reply_pop_up_alert,
