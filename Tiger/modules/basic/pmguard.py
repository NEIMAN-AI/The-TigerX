...
from Tiger.database.pmpermitdb import get_approved_users, pm_guard
import Tiger.database.pmpermitdb as Tiger
...
async def setpmmsg(client, message):
    ...
    if not arg:
        ...
        return
    if arg == "default":
        await Tiger.set_block_message(Zaid.BLOCKED)
        await message.edit("**Block message set to default**.")
        return
    await Tiger.set_block_message(f"`{arg}`")
    await message.edit(Tiger.set_block_message set**")
...
async def allow(client, message):
    ...
    await Tiger.allow_user(chat_id)
